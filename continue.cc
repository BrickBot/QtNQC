...
continue_obj::continue_obj(const int x0, const int y0,
			   QCanvas *canvas_arg, const int obj_arg_id):
  object(CONTINUE, x0, y0, canvas_arg, "", NULL, obj_arg_id)
{
  /*
  ** Draw a continue_obj.
  */
  ...
  if((text = new QCanvasText("GO", canvas)) == NULL)
    qtnqc_main::quit("Memory allocation failure", __FILE__, __LINE__,
		     EXIT_FAILURE);
...
QCanvasPolygon *continue_obj::getPolygon(void)
{
  return p;
}
...
void continue_obj::move(const QPoint &point, const bool subtract)
{
  QPointArray pa1(4);
...
bool continue_obj::move(const int direction, const int distance)
{
  QPoint pt;
