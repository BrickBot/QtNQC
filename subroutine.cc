...
subroutine::subroutine(const int x0, const int y0, QCanvas *canvas_arg,
		       const int type_arg, QPtrList<file_class> *list,
		       const int obj_id_arg, const QString &name_arg,
		       const int cwidth, const int cheight):
  object(type_arg, x0, y0, canvas_arg, "Subroutine", list, obj_id_arg,
	 cwidth, cheight)
{
  /*
  ** Draw a subroutine.
  */
  ...
  if(item_name.isEmpty())
    {
      for(i = 0; i < MAX_SUBROUTINES; i++)
	{
	  (void) snprintf(buffer, sizeof(buffer), "%d", i);

	  if(!qmain->getUsedSubroutineNames().contains(tmp.append(buffer)))
	    {
	      item_name = tmp;
	      qmain->getUsedSubroutineNames().append(tmp);
	      break;
	    }
	  else
	    tmp = "sub";
  ...
  if((text = new QCanvasText("s()", canvas)) == NULL)
    qtnqc_main::quit("Memory allocation failure", __FILE__, __LINE__,
		     EXIT_FAILURE);
...
subroutine::~subroutine()
{
  ...
  text = NULL;
  name = NULL;
  qmain->getUsedSubroutineNames().remove(item_name);
}
...
QCanvasPolygon *subroutine::getPolygon(void)
{
  return p;
}
...
void subroutine::move(const QPoint &point, const bool subtract)
{
  QPointArray pa(4);
...
bool subroutine::move(const int direction, const int distance)
{
  QPoint pt;
...
void subroutine::update(const QString &item_name_arg)
{
