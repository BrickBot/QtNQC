...
function::function(const int x0, const int y0, QCanvas *canvas_arg,
		   const int type_arg, const int cwidth, const int cheight):
  object(type_arg, x0, y0, canvas_arg, "Function", NULL, -1, cwidth, cheight)
{
  /*
  ** Draw a function.
  */
  ...
  for(i = 0; i < MAX_FUNCTIONS; i++)
    {
      ...
      if(!qmain->getUsedFunctionNames().contains(tmp.append(buffer)))
	{
	  item_name = tmp;
	  qmain->getUsedFunctionNames().append(tmp);
	  break;
	}
      else
	tmp = "func";
  ...
  if((text = new QCanvasText("f()", canvas)) == NULL)
    qtnqc_main::quit("Memory allocation failure", __FILE__, __LINE__,
		     EXIT_FAILURE);
...
function::~function()
{
  ...
  text = NULL;
  name = NULL;
  qmain->getUsedFunctionNames().remove(item_name);
}
...
QCanvasPolygon *function::getPolygon(void)
{
  return p;
}
...
void function::move(const QPoint &point, const bool subtract)
{
  QPointArray pa(4);
...
bool function::move(const int direction, const int distance)
{
  QPoint pt;
...
void function::update(const QString &item_name_arg)
{
